...
import {setProductStep, setProductValid} from '../../../../redux_tollkit/slices/makingSlice/makingProductForm'
import {setPasswordStep} from '../../../../redux_tollkit/slices/makingSlice/makingPasswordFrom'
import { useEffect } from "react"
// import { setCartCount } from "../../../../redux_tollkit/slices/cartSlice"
...
export default function PordutsOrderForm({products}:{products:CartItemInertFace[] | null}){


    const dispatch = useDispatch()

    const productStep = useSelector((state:any) => state.makkingProductForm.productStep)
    ...
    const handelNext = () => {
        if(products && products.length > 0 ){
            if(autorisStatus){
                dispatch(setProductStep(false))
                dispatch(setProductValid(true))
            }else{
                if(!passwordValid){
    ...
    const handelPrev = () => {
        ...
        dispatch(setProductValid(false))
    }
    ...
    <div className="MakingOrder-heading-status-wp">
       ...
       {productValid &&  <div className="MakingOrder-status-wp " onClick={handelPrev}>
            <p className="status-text" id="ProductsInTheOrderChange">изменить</p>
        ...
        {productStep && <>
            ...
            <div className="button-makorder-wp">
                <button className="button button-makorder" id="ProductsInTheOrderButton" onClick={handelNext}>далее</button>
            </div>
